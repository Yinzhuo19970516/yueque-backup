...
自定义 html，Shadow DOM 和 HTML 的模版  
...
通过 fetch 拿到 url 对应的 html 字符串，然后替换 head 和 body 标签为自定义标签  
...
- 若为样式链接，删除该 link,记录 href 内容，创建一个 style 标签插入 <micro-app-head>
- 创建 style 标签时会，给子应用的 style 标签添加作用域，实现样式隔离

处理 style 标签

- 给子应用的 style 标签加上作用域，前缀是 ${microApp.tagName}
-
